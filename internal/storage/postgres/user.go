...
func (r *UserRepository) FindByLogin(ctx context.Context, login string) (*model.User, error) {
	...
	if err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			// Возвращаем кастомную ошибку, чтобы на уровне сервиса ее можно было обработать.
			// TODO: Вынести ErrUserNotFound в пакет с ошибками домена.
...
func (r *UserRepository) Update(ctx context.Context, user *model.User) error {
	...
	`
	_, err := r.db.NamedExecContext(ctx, query, user)
	// TODO: Добавить проверку result.RowsAffected() для гарантии, что запись была обновлена.
	return err
}
