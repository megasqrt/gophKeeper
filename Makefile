...
build-client:
	...
	@GOOS=linux GOARCH=amd64 go build -o bin/gophkeeper-linux-amd64 client/cmd/main.go
	@echo "  Сборка для Windows (amd64)..."
	@GOOS=windows GOARCH=amd64 go build -o bin/gophkeeper-windows-amd64.exe client/cmd/main.go
	@echo "  Сборка для macOS (amd64)..."
	@GOOS=darwin GOARCH=amd64 go build -o bin/gophkeeper-darwin-amd64 client/cmd/main.go
	@echo "  Сборка для macOS (arm64)..."
	@GOOS=darwin GOARCH=arm64 go build -o bin/gophkeeper-darwin-arm64 client/cmd/main.go
	@echo "Сборка завершена! Бинарные файлы находятся в папке bin/"

clean-bin:
	@echo "Очистка папки bin..."
	@rm -rf bin
...
.PHONY: certs-all certs-ca certs-server certs-client clean-certs start startb stop proto run_s run_c build build-client clean-bin
